...
def learning_viz(classifier, output_folder):
    ...
    if classifier == 'KNN':
        ...
        # Process output
        param_clf__n_neighbors = np.array(grid_results[[
            "param_clf__n_neighbors"]].values, dtype='float64')
        mean_test_score = np.array(grid_results[[
            "mean_test_score"]].values, dtype='float64')
        mean_train_score = np.array(grid_results[[
            "mean_train_score"]].values, dtype='float64')

        # Create plots
        plt.plot(param_clf__n_neighbors,
                 mean_train_score, color='skyblue', linewidth=4)
        plt.plot(param_clf__n_neighbors,
                 mean_test_score, color='red', linewidth=4)
    ...
    if classifier == 'SVM':
        ...
        # Degree: 1 to 10
        # Gamma: 0 to 0.5
        svm_param_grid = {'clf__C': list(np.linspace(
        ...
        # Process output
        svm_params_clf__C = np.array(svm_grid_results[[
            "param_clf__C"]].values, dtype='float64')
        svm_params_clf__gamma = np.array(svm_grid_results[[
            "param_clf__gamma"]].values,  dtype='float64')
        svm_params_mean_train_score = np.array(svm_grid_results[[
            "mean_train_score"]].values,  dtype='float64')
        svm_params_mean_test_score = np.array(svm_grid_results[[
            "mean_test_score"]].values, dtype='float64')
        # Create interpolated surface
        f_train = sp.interpolate.interp2d(
            svm_params_clf__C, svm_params_clf__gamma,
            svm_params_mean_train_score, kind='linear', )
        f_test = sp.interpolate.interp2d(
            svm_params_clf__C, svm_params_clf__gamma,
            svm_params_mean_test_score, kind='linear', )
        ...
        # Degree: 1 to 10
        # Gamma: 0 to 0.5
        svm_param_grid = {'clf__C': list(np.linspace(1, 10, 25)),
                          'clf__kernel': ['rbf'],
        ...
        svm_params_mean_test_score = np.array(svm_grid_results[[
            "mean_test_score"]].values, dtype='float64')
        # Create interpolated surface
        f_train = sp.interpolate.interp2d(svm_params_clf__C,
                                          svm_params_clf__gamma,
                                          svm_params_mean_train_score,
                                          kind='linear', )
        f_test = sp.interpolate.interp2d(svm_params_clf__C,
                                         svm_params_clf__gamma,
                                         svm_params_mean_test_score,
                                         kind='linear', )
        ...
        svm_param_grid = {'clf__degree': list(
        ...
        # Process output
        svm_params_clf__deg = np.array(svm_grid_results[[
            "param_clf__degree"]].values, dtype='float64')
        svm_params_clf__gamma = np.array(svm_grid_results[[
            "param_clf__gamma"]].values,  dtype='float64')
        svm_params_mean_train_score = np.array(svm_grid_results[[
            "mean_train_score"]].values,  dtype='float64')
        ...
        # Create interpolated surface
        f_train = sp.interpolate.interp2d(
            svm_params_clf__deg, svm_params_clf__gamma,
            svm_params_mean_train_score, kind='linear', )
        f_test = sp.interpolate.interp2d(
            svm_params_clf__deg, svm_params_clf__gamma,
            svm_params_mean_test_score, kind='linear', )
        ...
        # Create linear plots
        fig = plt.subplots()
        plt.plot("param_clf__degree",
                 'mean_train_score',
                 data=svm_grid_results, color='skyblue', linewidth=4)
        plt.plot("param_clf__degree",
                 'mean_test_score',
                 data=svm_grid_results, color='red', linewidth=4)
        plt.legend(["train score", "test score"])
        plt.xlabel("Degree")
        plt.ylabel("f1 score")
    ...
    if classifier == 'DT':
        ...
        tree_param_grid = {'clf__max_depth': list(
            np.linspace(1, 50, 20, dtype=int)),
                           'clf__min_samples_split': list(
        ...
        # Process output
        tree_params_clf__max_depth = np.array(
            tree_grid_results[[
                "param_clf__max_depth"]].values, dtype='float64')
        tree_params_clf__min_samples_split = np.array(tree_grid_results[[
            "param_clf__min_samples_split"]].values,
                                                      dtype='float64')
        tree_params_clf__max_features = np.array(tree_grid_results[[
            "param_clf__max_features"]].values, dtype='float64')
        tree_params_mean_train_score = np.array(tree_grid_results[[
            "mean_train_score"]].values, dtype='float64')
        tree_params_mean_test_score = np.array(tree_grid_results[[
            "mean_test_score"]].values, dtype='float64')

        # Create 4D plot
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        img = ax.scatter(tree_params_clf__max_depth,
                         tree_params_clf__min_samples_split,
                         tree_params_clf__max_features,
                         c=np.reshape(
                             tree_params_mean_train_score, len(
                                 tree_params_mean_train_score)),
                         cmap=plt.hot())
        fig.colorbar(img)
        ...
        img = ax.scatter(tree_params_clf__max_depth,
                         ...
                         tree_params_clf__max_features,
                         c=np.reshape(
                             tree_params_mean_test_score, len(
                                 tree_params_mean_train_score)),
                         cmap=plt.hot())
        fig.colorbar(img)
...
def main():
    parser = argparse.ArgumentParser(
        description='Create machine learning visualizations')

    parser.add_argument('--classifier_type', type=str,
                        help='name of desired classifier', required=True)
    parser.add_argument('--output_folder', type=str,
                        help='name of the output folder for visualizations',
                        required=True)
